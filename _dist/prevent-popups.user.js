...
(function () {
  ...
  function isDebugEnabled() {
    return typeof GM_getValue === "function"
      ? GM_getValue(STORAGE_KEYS.debug, false)
      : false;
  }

  const log = (...args) =>
    isDebugEnabled() && console.log("[Autotask Popup Blocker]", ...args);
    ...
    return {
      tickets:
        typeof GM_getValue === "function"
          ? GM_getValue(STORAGE_KEYS.redirectTickets, true)
          : true,
      tasks:
        typeof GM_getValue === "function"
          ? GM_getValue(STORAGE_KEYS.redirectTasks, true)
          : true,
      kb:
        typeof GM_getValue === "function"
          ? GM_getValue(STORAGE_KEYS.redirectKB, true)
          : true,
    };
  }
  ...
  const redirectRules = [
    {
      ...
      getUrl: (matches, baseUrl) =>
        `${baseUrl}/Mvc/Projects/TaskDetail.mvc?taskId=${matches[1]}`,
    },
    {
      type: "tickets",
      pattern: /Code=OpenTicketDetail&ticketid=(\d+)/i,
      getUrl: (matches, baseUrl) =>
        `${baseUrl}/Mvc/ServiceDesk/TicketDetail.mvc?ticketId=${matches[1]}`,
    },
    {
      type: "kb",
      pattern: /Code=OpenKBArticle&id=(\d+)/i,
      getUrl: (matches, baseUrl) =>
        `${baseUrl}/Mvc/Knowledgebase/ArticleDetail.mvc/ArticlePage?articleId=${matches[1]}`,
    },
  ];
  ...
  function handleDetailPagePopup() {
    ...
    const detailMatch = currentUrl.match(
      /\/(TicketDetail|TaskDetail|ArticleDetail)\.mvc/i,
    );
    if (!detailMatch) {
      return false;
    }
    ...
    if (!isPopupUrl) {
      log(
        "Detail page opened in new tab (no workspace=False), allowing normal display",
      );
      return false;
    }
